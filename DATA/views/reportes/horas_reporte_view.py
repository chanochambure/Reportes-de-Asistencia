...
class horas_reporte_view(QDialog):
	...
	def horas_reporte_create(self):
		...
		#WIDGETS
		label_title = QLabel(REPORTES_HORAS_TITLE)
		...
		label_total_horas = QLabel(REPORTE_LABEL_TOTAL_HORAS)
	...
	def generate_reporte(self):
		if(self.var_singleton):
			...
			db=get_connection()
			if(db):
				self.reporte_matrix=reporte_horas.get_reporte_horas(self.worker.pin,datestr1,datestr2,db)
				self.minutos=reporte_horas.get_total_minutes(self.reporte_matrix)
	...
	def to_excel(self):
		...
		wb = Workbook()
		ws1 = wb.active
		ws1.title=REPORTES_HORAS_TITLE
		ws1["B1"]=REPORTES_HORAS_TITLE
